...
class ReservationLifecycleService:
    ...
    # === Приватные общие ядра и помощники ===

    async def _get_reservation_or_fail(
        self, reservation_id: int, owner: Optional[User] = None
    ) -> Reservation:
        """Получает резерв по ID; 404, если не найден или (для пользователя) чужой."""
        from fastapi import HTTPException, status
        reservation = await self.reservation_repo.get_by_id_with_details(reservation_id)
        if owner is not None:
            if not reservation or reservation.user_id != owner.id:
    ...
    async def _create_reservation_core(
        self, request: ReservationCreateRequest, user: User
    ) -> Reservation:
        """Общее ядро создания резерва: валидации, расчёт, сборка и сохранение.

        Вызывается внутри уже открытой транзакции (begin_nested);
        проверку прав/статуса пользователя выполняет вызывающий метод.
        """
        ...
        if request.selected_accessories:
            reservation.accessory_links = self._build_accessory_links(request.selected_accessories)
    ...
    async def _update_reservation_core(
        self, reservation: Reservation, request: ReservationUpdateRequest, user: User
    ) -> None:
        """Общее ядро обновления резерва: валидации, перерасчёт, обновление полей и сохранение.

        Вызывается внутри уже открытой транзакции (begin_nested);
        валидацию прав на редактирование выполняет вызывающий метод.
        """
        ...
        # confirm_date_adjustment: дата на выходной подтверждена (после диалога
        # подтверждения) — не отбиваем её повторным 409
    ...
    def _build_accessory_links(selected_accessories: dict) -> list:
        """Собирает связи резерва с аксессуарами, фильтруя некорректные ID."""
    ...
    @staticmethod
    def _equipment_display_names(reservation: Reservation) -> list:
        """Отображаемые имена оборудования резерва (для уведомлений)."""
        return [
    ...
    # === Публичные методы (пользовательский путь) ===
    ...
    async def update_user_reservation(
        ...
        try:
            # Все операции с БД теперь внутри одного блока транзакции
            async with self.db.begin_nested():
                reservation = await self._get_reservation_or_fail(reservation_id, owner=user)

                # Снапшот старых значений для уведомления об изменениях
                old_start_date = reservation.start_date
                old_end_date = reservation.end_date
                old_equipment_ids = set(reservation.equipment_ids)
                old_equipment_names = self._equipment_display_names(reservation)
    ...
    # === Публичные методы (администраторский путь) ===
    ...
    async def update_admin_reservation(
        ...
        """
        reservation = await self._get_reservation_or_fail(reservation_id)
