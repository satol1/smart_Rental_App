...
class TestBruteForceProtectionService:
    ...
    async def test_record_failed_attempt_already_blocked(self, brute_force_service, mock_security_audit_service):
        ...
        # Блокируем IP
        brute_force_service._block_ip(ip_address)
    ...
    def test_can_attempt_login_whitelisted(self, brute_force_service):
        """Тест проверки возможности входа для whitelisted IP."""
        can_attempt, reason = brute_force_service.can_attempt_login("127.0.0.1")
        
        assert can_attempt is True
        assert reason is None

    def test_can_attempt_login_blocked(self, brute_force_service):
        """Тест проверки возможности входа для заблокированного IP."""
        ip_address = "192.168.1.5"
        brute_force_service._block_ip(ip_address)
        
        can_attempt, reason = brute_force_service.can_attempt_login(ip_address)
        
        assert can_attempt is False
        assert "заблокирован" in reason.lower()

    def test_can_attempt_login_remaining_attempts(self, brute_force_service):
    ...
    def test_get_attempt_stats(self, brute_force_service):
    ...
    def test_get_attempt_stats_blocked(self, brute_force_service):
    ...
    def test_unblock_ip_success(self, brute_force_service):
        """Тест успешной разблокировки IP."""
        ip_address = "192.168.1.9"
        brute_force_service._block_ip(ip_address)
        
        result = brute_force_service.unblock_ip(ip_address)
        
        assert result is True
        assert ip_address not in brute_force_service.blocked_ips

    def test_unblock_ip_not_blocked(self, brute_force_service):
        """Тест разблокировки незаблокированного IP."""
        result = brute_force_service.unblock_ip("192.168.1.10")
        
        assert result is False

    # === ТЕСТЫ ДЛЯ cleanup_expired_data ===

    def test_cleanup_expired_data(self, brute_force_service):
    ...
    def test_get_all_blocked_ips(self, brute_force_service):
        """Тест получения всех заблокированных IP."""
        ip1 = "192.168.1.12"
        ip2 = "192.168.1.13"
        
        brute_force_service._block_ip(ip1)
        brute_force_service._block_ip(ip2)
