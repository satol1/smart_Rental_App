...
describe('PeriodFilter', () => {
    ...
    it('должен применять переданный className', () => {
        const { container } = render(_jsx(PeriodFilter, { className: "custom-class" }));
...
describe('PeriodFilterCompact', () => {
    ...
    it('должен рендериться без ошибок', () => {
        render(_jsx(PeriodFilterCompact, {}));
        expect(screen.getByRole('combobox')).toBeInTheDocument();
    ...
    it('должен применять переданный className', () => {
        const { container } = render(_jsx(PeriodFilterCompact, { className: "custom-class" }));
        expect(container.firstChild).toHaveClass('custom-class');
    });
});
describe('Интеграционные тесты PeriodFilter', () => {
    it('должен корректно работать с полным циклом выбора периода', async () => {
        ...
        let mockPeriodType = null;
        mockSetPeriodType.mockImplementation((value) => { mockPeriodType = value; });
        useOrderFilterStore.mockImplementation(() => ({
            periodType: mockPeriodType,
            periodOffset: 0,
            setPeriodType: mockSetPeriodType,
            setPeriodOffset: mockSetPeriodOffset
        }));
        const { rerender } = render(_jsx(PeriodFilter, {}));
