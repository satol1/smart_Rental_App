...
import { SkeletonCard, SkeletonList, SkeletonTable, } from '@/components/ui/skeleton-list';
describe('SkeletonList / SkeletonCard / SkeletonTable', () => {
    it('SkeletonList рендерит count карточек', () => {
        render(_jsx(SkeletonList, { count: 5 }));
    ...
    it('SkeletonTable рендерит заголовок и ровно rows строк', () => {
        const rows = 4;
        const columns = 3;
        render(_jsx(SkeletonTable, { rows: rows, columns: columns }));
