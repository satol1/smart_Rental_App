...
export class AccessoryService {
     ...
     */
    static async getAllAccessories(skip, limit) {
     ...
     */
    static async createAccessory(input) {
     ...
     * Обновляет аксессуар
     */
    static async updateAccessory(input) {
     ...
     */
    static async getAccessoriesForEquipment(equipmentId) {
     ...
     */
    static filterAccessories(accessories, filters) {
        return accessories.filter(accessory => {
            // Фильтр по поиску
            if (filters.search) {
                const searchLower = filters.search.toLowerCase();
                const matchesSearch = accessory.name.toLowerCase().includes(searchLower) ||
     ...
     */
    static groupAccessoriesByType(accessories) {
     ...
     */
    static sortAccessories(accessories, sortBy, sortOrder = 'asc') {
     ...
     */
    static validateAccessoryData(input) {
        const errors = [];
        if (!input.name?.trim()) {
     ...
     */
    static calculateAccessoriesTotal(selectedAccessories, allAccessories) {
        let total = 0;
