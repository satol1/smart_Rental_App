 ...
 */
export const MAX_RESERVATIONS_BY_STATUS = {
    [USER_STATUS.NEW]: 2,
 ...
 */
export const EDIT_RESTRICTION_DAYS = {
    [USER_STATUS.NEW]: 2,
 ...
 */
export function isInReservationGracePeriod(createdAt) {
    if (!createdAt)
        return false;
 ...
 */
export function remainingGraceHours(createdAt) {
    if (!createdAt)
        return 0;
    const created = new Date(createdAt);
    if (isNaN(created.getTime()))
 ...
 */
export function canUserEditReservation(userStatus, daysUntilStart, createdAt) {
    if (!userStatus)
        return false;
    ...
    if (restrictionDays === 999)
        return false; // Персона НонГрата
    if (daysUntilStart < 0 || Number.isNaN(daysUntilStart))
        return false; // прошло
    if (isInReservationGracePeriod(createdAt))
        return true; // grace-период
 ...
 */
export function canUserCancelReservation(userStatus, daysUntilStart, createdAt) {
 ...
 */
export function getMaxReservationsForStatus(userStatus) {
    if (!userStatus)
        return 0;
 ...
 */
export function mapLegacyUserStatus(status) {
    if (!status)
        return null;
