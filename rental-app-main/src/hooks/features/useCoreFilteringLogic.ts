...
interface UseCoreFilteringLogicProps {
  allEquipment: Equipment[];
  allAssociations: Association[];
  filters: FilterState;
}

interface UseCoreFilteringLogicReturn {
 ...
 */
export function useCoreFilteringLogic({
  allEquipment,
  allAssociations,
  filters
}: UseCoreFilteringLogicProps): UseCoreFilteringLogicReturn {
  ...
  const filteredEquipment = useMemo(() => {
    ...
    if (associationId) {
      ...
      if (association && association.equipment_ids.length > 0) {
        filtered = filtered.filter(equipment => 
          association.equipment_ids.includes(equipment.id)
        );
      } else {
  ...
  const availableTypes = useMemo(() => {
    ...
    if (query && query.trim()) {
      const searchQuery = query.toLowerCase().trim();
      equipmentForTypes = equipmentForTypes.filter(equipment => 
        equipment.name.toLowerCase().includes(searchQuery) ||
        equipment.brand.toLowerCase().includes(searchQuery) ||
        equipment.equipment_type.toLowerCase().includes(searchQuery) ||
        equipment.description?.toLowerCase().includes(searchQuery) ||
  ...
  const availableBrands = useMemo(() => {
    ...
    if (query && query.trim()) {
      const searchQuery = query.toLowerCase().trim();
      equipmentForBrands = equipmentForBrands.filter(equipment => 
        equipment.name.toLowerCase().includes(searchQuery) ||
        equipment.brand.toLowerCase().includes(searchQuery) ||
        equipment.equipment_type.toLowerCase().includes(searchQuery) ||
        equipment.description?.toLowerCase().includes(searchQuery) ||
  ...
  const availableAssociations = useMemo(() => {
    ...
    if (query && query.trim()) {
      const searchQuery = query.toLowerCase().trim();
      equipmentForAssociations = equipmentForAssociations.filter(equipment => 
        equipment.name.toLowerCase().includes(searchQuery) ||
        equipment.brand.toLowerCase().includes(searchQuery) ||
        equipment.equipment_type.toLowerCase().includes(searchQuery) ||
        equipment.description?.toLowerCase().includes(searchQuery) ||
    ...
    const relevantAssociations = allAssociations.filter(association => 
      association.equipment_ids.some(equipmentId => filteredEquipmentIds.has(equipmentId))
    );
