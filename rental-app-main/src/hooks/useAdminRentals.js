...
import { RentalService } from "@/core/services";
...
export function useAdminRentals(params = {}) {
...
export function useConvertReservationToRental() {
    ...
    return useMutation({
        mutationFn: (payload) => RentalService.convertReservationToRental(payload),
...
export function useDeleteAdminRental() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (rentalId) => RentalService.deleteAdminRental(rentalId),
        onSuccess: () => {
...
export function useReturnRental() {
    ...
    return useMutation({
        ...
        mutationFn: (payload) => RentalService.returnRental(payload),
        onSuccess: () => {
...
export function useRevertRentalToReservation() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ rentalId, refundPrepayment }) => RentalService.revertRentalToReservation(rentalId, refundPrepayment),
        onSuccess: () => {
...
export function useCreateAdminRentalFromScratch() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (data) => RentalService.createRentalFromScratch(data),
        onSuccess: async (newRentalData) => {
            // `newRentalData` уже содержит данные аренды (AdminRentalOut) от RentalService
            // Проверяем, что данные корректны
            if (!newRentalData || !newRentalData.id) {
...
export function useUpdateAdminRental() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (payload) => RentalService.updateAdminRental(payload),
        onSuccess: () => {
