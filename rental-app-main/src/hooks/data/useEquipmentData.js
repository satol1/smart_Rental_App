...
export const useEquipmentData = (filters) => {
    const { addToReservationMode } = useReserveStore();
    // Загружаем пагинированный список оборудования
    const { data: equipmentPages, fetchNextPage, hasNextPage, isLoading, isFetchingNextPage } = useEquipment(filters);
    // Извлекаем плоский массив оборудования
    const equipment = useMemo(() => equipmentPages?.pages.flatMap(page => page.items) ?? [], [equipmentPages]);
    ...
    const allEquipmentIds = useMemo(() => {
        ...
        const packEquipmentIds = equipmentPages?.pages.flatMap(page => page.packs?.flatMap(pack => pack.equipment_ids) || []) || [];
    ...
    const getEquipmentData = useCallback((equipmentId) => {
