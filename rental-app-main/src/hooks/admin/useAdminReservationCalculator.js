 ...
 */
export function useAdminReservationCalculator(watch, allEquipment = []) {
    ...
    const { promoCodeInput: promoCode, setPromoCodeInput: setPromoCode, appliedPromoCode, applyPromoCode: storeApplyPromoCode, removePromoCode: storeRemovePromoCode, promoCodeMessage } = usePromoCodeStore();
    ...
    const equipmentForCalc = useMemo(() => allEquipment.filter(e => watchedEquipmentIds.includes(e.id)), [allEquipment, watchedEquipmentIds]);
