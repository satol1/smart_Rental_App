...
vi.mock('@/core/services/HolidayService', () => ({
    HolidayService: {
        getHolidayList: (...args: unknown[]) => getHolidayList(...args),
        createHoliday: (...args: unknown[]) => createHoliday(...args),
...
describe('useHolidays', () => {
    ...
    it('загружает выходные и преобразует даты через select', async () => {
        ...
        const { result } = renderHook(
            () => useHolidays(new Date(2026, 0, 1), new Date(2026, 0, 31)),
        ...
        expect(getHolidayList).toHaveBeenCalledWith(
