...
export function useRentalReturnForm({ rental, onClose }) {
    ...
    const [checkedAccessories, setCheckedAccessories] = useState(new Set());
    ...
    const accessoriesByEquipment = useMemo(() => {
        if (!rental?.accessory_links)
            return {};
        return rental.accessory_links.reduce((acc, current) => {
