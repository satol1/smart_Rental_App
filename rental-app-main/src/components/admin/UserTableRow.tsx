        ...
        <TableRow key={user.id}>
            <TableCell className="font-medium">
                <div className="flex items-center gap-2">
                    {user.role === "admin" && <ShieldCheck className="w-4 h-4 text-destructive" />}
                    {user.role === "manager" && <Shield className="w-4 h-4 text-primary" />}
                    {user.full_name}
                ...
                <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                        {user.id !== currentUser?.id ? (
                            <>
                                <Button variant="outline" size="icon" onClick={() => onAddPayment(user)} className="h-8 w-8 text-success border-success/40 hover:bg-success-soft hover:text-success" aria-label="Пополнить баланс пользователя">
