...
export function CalendarCell({ cellData, equipment, isHighlighted, isUnderRepair, onSelect, onShowDetails, onNavigate, isActionAllowed }) {
    ...
    if (!cellData || !cellData.group_id || !cellData.order_type || !cellData.user_id) {
        ...
        return _jsx("div", { className: `w-full h-6 rounded-md ${bgColor}` });
    }
    ...
    return (_jsxs(Popover, { children: [_jsx(PopoverTrigger, { asChild: true, children: _jsx("div", { onClick: handleSingleClick, onDoubleClick: handleDoubleClick, className: cn("w-full h-6 rounded-md transition-colors cursor-pointer", bgColor, isHighlighted && "ring-2 ring-sky-500 ring-inset"), title: `${status} #${orderId}` }) }), isActionAllowed && (_jsx(PopoverContent, { side: "top", align: "center", className: "w-auto p-2", onClick: (e) => e.stopPropagation(), children: _jsxs("div", { className: "space-y-2", children: [_jsxs("div", { className: "text-sm font-medium", children: [order_type === 'reservation' ? 'Резерв' : 'Аренда', " #", orderId] }), _jsxs(Button, { onClick: () => onNavigate(eventInfo), size: "sm", className: "w-full", children: [_jsx(ArrowRight, { className: "w-3 h-3 mr-1" }), " \u041F\u0435\u0440\u0435\u0439\u0442\u0438"] })] }) }))] }));
}
