...
export default function ReservationCard({ id, equipment, start_date, end_date, status, onRemoveItem, onCancel, cancelDisabled, onRepeat, autoStartEdit = false, total_cost, discount_amount, promo_code, selected_accessories, accessory_links, rental_id, fullEquipmentData, created_at }) {
    ...
    const reservationForProvider = useMemo(() => ({
    ...
    return (_jsx("div", { ref: elementRef, className: `transition-all duration-300 ${highlightClasses}`, children: isEditing ? (
        // ✨ 3. Оборачиваем компонент редактирования в наш новый провайдер.
        _jsx(ReservationEditProvider, { reservation: reservationForProvider, initialEquipmentForDisplay: equipment, onFullCancellation: onCancel, isAdminContext: isAdminOrManager, 
            // ✅ УПРОЩЕННАЯ ЛОГИКА: Просто закрываем редактирование
            onFinishEditing: () => {
                console.log("🔧 [ReservationCard] onFinishEditing вызван для резерва #", id);
                setIsEditing(false);
            }, children: _jsx(EditableReservationCard, {}) })) : (_jsx(ViewReservationCard, { id: id, equipment: fullEquipmentData || equipment.map(eq => ({ id: eq.id, name: eq.label })), start_date: start_date, end_date: end_date, status: status, 
            // ✨ 5. Кнопка "Редактировать" теперь просто переключает локальное состояние.
            onEdit: status === 'active' ? () => setIsEditing(true) : undefined, onCancel: onCancel, onRemoveItem: onRemoveItem, onRepeat: onRepeat, cancelDisabled: cancelDisabled, total_cost: total_cost, discount_amount: discount_amount, promo_code: promo_code, accessory_links: accessory_links, rental_id: rental_id, created_at: created_at })) }));
}
