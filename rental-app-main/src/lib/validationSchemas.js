...
const validateRussianPhone = (phone) => {
    if (!phone)
        return true; // Пустое значение разрешено для опциональных полей
...
export const equipmentCreateSchema = z.object({
    ...
    daily_rate: z.preprocess((val) => (val === "" || val === null || val === undefined) ? 0 : parseFloat(String(val)), z.number({ invalid_type_error: "Стоимость должна быть числом." })
...
export const accessorySchema = z.object({
    ...
    price: z.preprocess((value) => (value === "" ? undefined : value), z.coerce.number({ invalid_type_error: "Цена должна быть числом." })
...
export const promoCodeFormSchema = z.object({
    ...
    max_uses: z.preprocess((val) => (val === "" || val === null || val === undefined) ? undefined : parseInt(String(val), 10), z.number().positive("Значение должно быть больше 0").optional()),
    max_uses_per_user: z.preprocess((val) => (val === "" || val === null || val === undefined) ? undefined : parseInt(String(val), 10), z.number().positive("Значение должно быть больше 0").optional()),
    min_order_amount: z.preprocess((val) => (val === "" || val === null || val === undefined) ? undefined : parseFloat(String(val)), z.number().positive("Значение должно быть больше 0").optional()),
