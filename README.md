...
### Правило 3-2-1 (этап 6.2 аудита 2026-09-12)

Планировщик `scripts/backup_loop.sh` поддерживает шифрование дампов и копию
на внешний носитель — включаются переменными в `.env`:

```env
# Шифрование дампов БД (openssl aes-256-cbc + pbkdf2): в db_backup/ больше
# нет открытых хэшей паролей и ПД. Ключ храните ВНЕ сервера (менеджер секретов).
BACKUP_ENCRYPTION_KEY=<длинная случайная строка>

# Каталог внешнего носителя ВНУТРИ контейнера db-backup: внешний диск,
# NFS- или rclone-маунт (например, копия в S3/Yandex Object Storage через
# `rclone mount` на хосте). Раскомментируйте volume в docker-compose.yml.
EXTERNAL_BACKUP_DIR=/external
```

**Тест-рестор** (регулярность бэкапов без проверки восстановления не считается
защитой): восстанавливает свежайший дамп в одноразовую базу и проверяет
ключевые таблицы, прод-база не затрагивается:

```bash
docker compose --profile backup-test run --rm backup-restore-test
```

Запускайте по расписанию хоста (cron) минимум раз в неделю; скрипт возвращает
ненулевой код при провале — удобно для алертов.
