...
> - **Этап 4 — выполнен** (`9027b59`): тёмная тема домигрирована на токены (~35 файлов), EquipmentDialog модальный с guard несохранённых изменений, сортировка админ-таблиц (SortableTableHead+useTableSort, серверная у пользователей/аксессуаров), серверный поиск пользователей/аксессуаров + debounce, 11 window.confirm → ConfirmationDialog, cookie-баннер/футер, иконки типов каталога, мелочи UX (Войти/44px/aria/SkeletonList/MoneyText/панель дат); бэк: search/sort у /admin/users и /accessories (+11 тестов); schema.d.ts регенерирован (закрыл дрейф CI от этапов 0-3).
> - **Этап 5 — выполнен** (`e10356b`): codegen-типы в дело (schemas.ts, деривации Rental/Reservation/Dashboard/Auth + RegisterResponse-схема на бэке; CI-правило check:api-types), инвалидация публичного каталога из админ-мутаций, праздники одним слоем (holidayStore удалён, useHolidays на react-query), «Мои резервы» infinite + промокоды постранично, мёртвый код удалён (DI-слой, PromoCodeService и др.), orderFilterStore изолирован по контекстам (блокер useShallow-цикла пойман аудитом и закрыт регресс-тестом), ресинк useReservationState, useMemo провайдеров, 19 финансовых тестов + e2e-шаг редактирования резерва; e2e прогнан локально против реального бэка.
> - **Этап 6 — выполнен** (`c2b4531`): ротация refresh (reuse-детект + fence-отзыв всех сессий с grace 30с от self-DoS, access TTL 15м, +10 тестов), бэкапы 3-2-1 (шифрование openssl iter=600k, внешний носитель, тест-рестор; аудитом пойманы и закрыты: тест-рестор дропал прод, openssl отсутствовал в alpine — свой образ), сети edge/internal, пины actions по SHA, CSRF Secure/ротация логов/uuid request-id/политика uploads, гигиена ошибок фронта (401 по статусу, один канал).
>
> Тесты на момент завершения: бэкенд 1514 passed, фронтенд 366 passed, eslint/tsc/ruff чисто, e2e smoke passed (локально против реального бэкенда).
