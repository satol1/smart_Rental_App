...
# Скрипт для восстановления базы данных PostgreSQL из бэкапа в Docker с проверкой кириллицы
...
FILE_ENCODING=$(file -bi "$TEMP_DIR/restore.sql" | grep -o "charset=[^;]*" | cut -d= -f2)
log "Кодировка файла бэкапа: $FILE_ENCODING"
...
log "Восстанавливаем базу данных с правильной кодировкой UTF-8..."
...
if [ "$TABLE_COUNT" -gt 0 ]; then
    ...
    # Тестируем кириллицу
    log "Тестируем сохранение кириллических символов..."
    CYRILLIC_TEST=$(docker-compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT 'Тест кириллицы: Привет мир!' as test;" 2>/dev/null | xargs)
    
    if [[ "$CYRILLIC_TEST" == *"Привет"* ]]; then
        success "Кириллические символы работают корректно: $CYRILLIC_TEST"
    else
        warning "Возможны проблемы с кириллицей: $CYRILLIC_TEST"
    fi
    
    # Проверяем наличие данных в таблицах
    log "Проверяем наличие данных в основных таблицах..."
    
    # Проверяем таблицу пользователей
    USER_COUNT=$(docker-compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM users;" 2>/dev/null | xargs || echo "0")
    if [ "$USER_COUNT" -gt 0 ]; then
        log "Пользователи: $USER_COUNT записей"
    fi
    
    # Проверяем таблицу оборудования
    EQUIPMENT_COUNT=$(docker-compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM equipment;" 2>/dev/null | xargs || echo "0")
    if [ "$EQUIPMENT_COUNT" -gt 0 ]; then
        log "Оборудование: $EQUIPMENT_COUNT записей"
    fi
    
    # Проверяем таблицу резерваций
    RESERVATION_COUNT=$(docker-compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM reservations;" 2>/dev/null | xargs || echo "0")
    if [ "$RESERVATION_COUNT" -gt 0 ]; then
        log "Резервации: $RESERVATION_COUNT записей"
    fi
...
log "Финальная проверка кириллических символов..."
FINAL_TEST=$(docker-compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT 'Финальный тест: Кириллица работает!' as final_test;" 2>/dev/null | xargs)

if [[ "$FINAL_TEST" == *"Кириллица"* ]]; then
    success "✅ Кириллические символы полностью восстановлены!"
    success "✅ База данных готова к использованию"
else
    warning "⚠️  Возможны проблемы с кириллицей в восстановленной базе"
fi

success "Процесс восстановления завершен успешно!"
log "База данных готова к использованию с сохранением кириллицы"
